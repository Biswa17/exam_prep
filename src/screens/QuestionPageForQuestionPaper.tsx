...
const QuestionPageForQuestionPaper: React.FC = () => {
  ...
  const [pageNumber, setPageNumber] = useState(() => {
    ...
    const saved = localStorage.getItem(`pageNumber_qp_${questionPaperId}`);
  ...
  const [selectedAnswers, setSelectedAnswers] = useState<{
    ...
    const saved = localStorage.getItem(`selectedAnswers_qp_${questionPaperId}`);
    return saved ? JSON.parse(saved) : {};
  });

  const [isSubmitted, setIsSubmitted] = useState<{ [key: string]: boolean }>(
    () => {
      const saved = localStorage.getItem(`isSubmitted_qp_${questionPaperId}`);
      return saved ? JSON.parse(saved) : {};
    }
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      `selectedAnswers_qp_${questionPaperId}`,
      JSON.stringify(selectedAnswers)
    );
    localStorage.setItem(`isSubmitted_qp_${questionPaperId}`, JSON.stringify(isSubmitted));
    localStorage.setItem(`pageNumber_qp_${questionPaperId}`, pageNumber.toString());
  }, [selectedAnswers, isSubmitted, pageNumber, questionPaperId]);
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const data = await apiRequest<{
          questions: any[];
          total_count: number;
        }>(`/api/sf/questions/question-paper/${questionPaperId}?page=${pageNumber}${statusParam}`, "GET");
  ...
  }, [questionPaperId, pageNumber, filters]);
  ...
  const handleBackToExamDetails = async () => {
    try {
      await submitUserAnswers(); // Call the common function
    } catch (error) {
      ...
      setSelectedAnswers({});
      setIsSubmitted({});
      localStorage.removeItem(`selectedAnswers_qp_${questionPaperId}`);
      localStorage.removeItem(`isSubmitted_qp_${questionPaperId}`);
      localStorage.removeItem(`pageNumber_qp_${questionPaperId}`); // Also remove page number
          ...
          <div className="alert alert-info mt-4" role="alert" style={{
            ...
            <h4 style={{ marginBottom: '1rem' }}>
              {filters.solved && !filters.unsolved
                ...
                : "No questions available for this question paper"}
            </h4>
...
export default QuestionPageForQuestionPaper;
